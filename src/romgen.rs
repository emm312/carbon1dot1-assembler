...
pub fn generate_schem<W: std::io::Write>(writer: &mut W, content: &[u8], bytes_per_page: usize) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for ir in 0..8 {
            for ih in 0..8 {
                schem.set_block_entity(
                    ...
                    BlockEntity::Barrel { items: utils::barrel_ss(
                        ((page[ir] >> (7 - ih)) +
                        ((page[ir + 8] >> (7 - ih)) << 1) +
                        ((page[ir + 16] >> (7 - ih)) << 2) +
                        ((page[ir + 24] >> (7 - ih)) << 3)) as usize
                    ) }
                );
                y += 2;
            }
